...
public final class Twofish_Algorithm // implicit no-argument constructor
{
    ...
    public static synchronized Object makeKey(byte[] k)
        ...
        if (DEBUG && debuglevel > 7) {
            ...
            for (i = 0; i < k64Cnt; i++) {
                System.out.println("0x" + intToString(k32o[i]) + "  0x" + intToString(k32e[i]) + " --> 0x"
                        + intToString(sBoxKey[k64Cnt - 1 - i]));
            }
            System.out.println();
    ...
    public static byte[] blockDecrypt(byte[] in, int inOffset, Object sessionKey) {
        ...
        for (int R = 0; R < ROUNDS; R += 2) {
            ...
            x2 ^= t0 + t1 + sKey[k--];
            if (DEBUG && debuglevel > 6)
                System.out.println("PT" + (ROUNDS - R - 1) + "=" + intToString(x2) + intToString(x3) + intToString(x0)
                        + intToString(x1));
        }
        x0 ^= sKey[INPUT_WHITEN];
        x1 ^= sKey[INPUT_WHITEN + 1];
